...
export class CategoriaPage implements OnInit {
  ...
  save(form){
    ...
    if(this.categoria.idCategoria){
      this.mermaService.updatecategoria(this.categoria)
    }else{

        this.categoria.usuarioId = (this.currentUser.length > 0) ?  this.currentUser : '1'; 

        this.mermaService.addcategoria(this.categoria).then(res=>{
        })
